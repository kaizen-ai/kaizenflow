...
class ReplayedMarketData(mdabmada.AbstractMarketData):
    ...
    def _get_last_end_time(self) -> Optional[pd.Timestamp]:
        ...
        _LOG.debug(hpandas.df_to_str(df, print_shape_info=True, tag="after get_data"))
        if df.empty:
            ret = None
        else:
            ret = df.index.max()
...
def save_market_data(
    ...
    _LOG.debug(hpandas.df_to_str(rt_df, print_dtypes=True, print_shape_info=True, tag="rt_df"))
    #
    _LOG.info("Saving ...")
    compression = None
...
def load_market_data(
    ...
    **kwargs: Dict[str, Any],
    ...
    _LOG.debug(hpandas.df_to_str(df, print_dtypes=True, print_shape_info=True, tag="df"))
    return df
