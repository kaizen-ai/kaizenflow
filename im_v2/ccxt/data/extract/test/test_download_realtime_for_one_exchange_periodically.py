...
class TestDownloadRealtimeForOneExchangePeriodically1(hunitest.TestCase):
    def test_amount_of_downloads(self) -> None:
        ...
        cmd = cmd.format(start_time=start_time.strftime("%Y-%m-%d %H:%M:%S"),
                         stop_time=stop_time.strftime("%Y-%m-%d %H:%M:%S"))
        process = subprocess.Popen(args=cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
        stdout, _ = process.communicate()
        # Check return value.
        self.assertEqual(process.returncode, 0)
        # Check amount of downloads by parsing output.
        self.assertEqual(str(stdout).count(download_started_marker), expected_downloads_amount)
