"""
Production live trading system DAG.
"""
...
# TODO(Nina): consider parsing tag using `extract_components_from_filename()`.
_TAG = "config3"
_DAG_BUILDER_CTOR = (
...
_DAG_CONFIG_PARAMS = {
    "run": [
        ...
        '--set_config_value \'("dag_config","resample","transformer_kwargs","rule"),(str("120T"))\'',
        '--set_config_value \'("dag_config","generate_feature","in_col_groups"),([("r_lr.c",)])\'',
        '--set_config_value \'("market_data_config","days"),(pd.Timedelta(str("121T")))\'',
        '--set_config_value \'("market_data_config","universe_version"),(str("v8.1"))\'',
        '--set_config_value \'("trading_period"),(str("120T"))\'',
        '--set_config_value \'("portfolio_config", "pricing_method"),(str("last"))\'',
        '--set_config_value \'("process_forecasts_node_dict","process_forecasts_dict","order_config","order_duration_in_mins"),(int(120))\'',
        ...
        '--set_config_value \'("process_forecasts_node_dict","process_forecasts_dict","optimizer_config","params"),({"dollar_neutrality_penalty": float(0.0), "constant_correlation": float(0.5), "constant_correlation_penalty": float(0.0), "relative_holding_penalty": float(0.0), "relative_holding_max_frac_of_gmv": float(0.1), "target_gmv": float(1000.0), "target_gmv_upper_bound_penalty": float(0.0), "target_gmv_hard_upper_bound_multiple": float(1.05), "transaction_cost_penalty": float(0.25), "solver": str("ECOS")})\'',
    ],
    "reconcile": [
        '--set-config-values \'("dag_property_config","force_free_nodes"),(bool(False));("dag_property_config","debug_mode_config","profile_execution"),(bool(True));("process_forecasts_node_dict","process_forecasts_dict","order_config","order_type"),(str("price@start"));("dag_config","generate_feature","in_col_groups"),([("r_lr.c",)]);("dag_config","resample","transformer_kwargs","rule"),(str("120T"));("trading_period"),(str("120T"));("market_data_config","days"),(pd.Timedelta(str("121T")));("portfolio_config", "pricing_method"),(str("last"));("process_forecasts_node_dict","process_forecasts_dict","order_config","order_duration_in_mins"),(int(120));("process_forecasts_node_dict","process_forecasts_dict","optimizer_config","backend"),(str("batch_optimizer"));("process_forecasts_node_dict","process_forecasts_dict","optimizer_config","params"),({"dollar_neutrality_penalty": float(0.0), "constant_correlation": float(0.5), "constant_correlation_penalty": float(0.0), "relative_holding_penalty": float(0.0), "relative_holding_max_frac_of_gmv": float(0.1), "target_gmv": float(1000.0), "target_gmv_upper_bound_penalty": float(0.0), "target_gmv_hard_upper_bound_multiple": float(1.05), "transaction_cost_penalty": float(0.25), "solver": str("ECOS")});("market_data_config","universe_version"),(str("v8.1"));("process_forecasts_node_dict","process_forecasts_dict","liquidate_at_trading_end_time"),(bool(True))\''
    ],
}
_DAG_BAR_DURATION_MIN = {"C11a": 120}
_UNIVERSE = "v8.1"
_LIVENESS = "CANDIDATE"
_INSTANCE_TYPE = "PROD"
_VERBOSITY = "INFO"
...
_SECRET_ID = 9
_DAG_DESCRIPTION = "Prod Live System DAG"
...
_RUN_DURATION_IN_SECS = 79200
_SCHEDULE = "30 11 * * *"
# Uncomment to turn off the schedule.
# _SCHEDULE = None
_VOLATILITY_MULTIPLE = "[float(1.5), float(0.7), float(0.7), float(0.7), float(0.7), float(0.7), float(0.7), float(0.7), float(0.7), float(0.6), float(0.6), float(0.5), float(0.25), float(0.25), float(0.25)] + ([10.0]*705)"
