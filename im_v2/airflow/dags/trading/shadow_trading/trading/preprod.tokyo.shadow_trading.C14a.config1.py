...
import im_v2.airflow.dags.trading.shadow_trading.trading.preprod.tokyo.shadow_trading.C14a.config1 as imvadtsttptstcc
"""
...
_DAG_BUILDER = "C14a"
_TAGS = {"C14a": "config1"}
_DAG_BUILD_CTOR_SUFFIX = {"C14a": ""}
_DAG_REPO = {"C14a": "lemonade"}
_DAG_CONFIG_PARAMS = {
    "C14a": {
        "config1": {
            "run": [
                ...
                '--set_config_value \'("process_forecasts_node_dict","process_forecasts_dict","optimizer_config","params"),({"dollar_neutrality_penalty": float(0.0), "constant_correlation": float(0.5), "constant_correlation_penalty": float(0.0), "relative_holding_penalty": float(0.0), "relative_holding_max_frac_of_gmv": float(0.1), "target_gmv": float(100000.0), "target_gmv_upper_bound_penalty": float(0.0), "target_gmv_hard_upper_bound_multiple": float(1.05), "transaction_cost_penalty": float(0.35), "solver": str("ECOS")})\'',
...
start_timestamp = "{{{{ align_to_grid(dag_run.get_task_instance('start_dag').start_date \
    ...
    _DAG_BAR_DURATION_MIN[_DAG_BUILDER]
)
...
end_timestamp = "{{{{ ((align_to_grid(dag_run.get_task_instance('start_dag').start_date \
    ...
    _DAG_BAR_DURATION_MIN[_DAG_BUILDER], _DAG_BAR_DURATION_MIN[_DAG_BUILDER]
)
...
start_task >> system_run_task >> system_reconcile_task >> end_task >> end_dag
