...
download_command = [
    ...
    "--start_time '{{ macros.datetime.now(dag.timezone).replace(second=0, microsecond=0) + macros.timedelta(minutes=(var.value.rt_data_download_standby_min | int) - 1, seconds=3, milliseconds=500) }}'",
    "--stop_time '{{ data_interval_end + macros.timedelta(minutes=(var.value.rt_data_download_run_for_min | int) + (var.value.rt_data_download_standby_min | int) + 2) }}'",
    "--method 'websocket'",
    "--watch_multiple_symbols",
    # For Tokyo we want to insert as frequently as possible.
