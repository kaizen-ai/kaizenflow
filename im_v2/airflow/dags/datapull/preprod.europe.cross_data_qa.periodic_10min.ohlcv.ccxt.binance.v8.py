...
# The task compares the ad-hoc downloaded data with the contents of
...
_QA_JOBS = [
    ...
    #    "dataset_signature2": "periodic_10min.airflow.downloaded_1min.parquet.ohlcv.futures.v7_6.ccxt.okx.v1_0_0",
    #    "add_invoke_params": []
    #},
]
# Shared location to store the reconciliaiton notebook into
_QA_NB_DST_DIR = os.path.join("{{ var.value.efs_mount }}", _STAGE, "data_qa", "period_10min")
...
for job in _QA_JOBS:
    ...
    downloading_task = aiutecop.get_ecs_run_task_operator(
        dag,
        _STAGE,
        f"download_data.{dataset_signature_10min[8]}",
        curr_bash_command,
    ...
    comparing_task = aiutecop.get_ecs_run_task_operator(
        dag,
        _STAGE,
        f"data_qa.{dataset_signature_10min[8]}",
        curr_invoke_cmd,
        _ECS_TASK_DEFINITION,
        512,
        2048,
