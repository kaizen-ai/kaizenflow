...
class Extractor(abc.ABC):
    ...
    def download_websocket_data(self, data_type: str, exchange_id: str, currency_pair: str, **kwargs: Any
    ) -> Dict:
        """
        Download exchange data using websocket based approach assuming
         the websocket corresponding to the particular exchange and currency_pair
        ...
        if data_type == "ohlcv":
            ...
            data = self._download_websocket_trades(exchange_id, currency_pair, **kwargs)
        else:
            hdbg.dfatal(
                f"Unknown data type {data_type}. Possible data types: ohlcv, bid_ask, trades"
            )
    ...
    async def subscribe_to_websocket_data(self, data_type: str, exchange_id: str, currency_pair: str, **kwargs: Any
        ...
        if data_type == "ohlcv":
            ...
            await self._subscribe_to_websocket_trades(exchange_id, currency_pair, **kwargs)
        else:
            hdbg.dfatal(
                f"Unknown data type {data_type}. Possible data types: ohlcv, bid_ask, trades"
            )
