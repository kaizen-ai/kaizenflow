...
class TestResampleBidAskData(hunitest.TestCase):
    ...
    def test_resample_bid_ask_data2(self) -> None:
        ...
        decimal = 4
        # Check `bid_price`.
        bid_price1 = (387.2 * 60.2 + 386.68 * 32.21 + 390.12 * 45.5) / (60.2 + 32.21 + 45.5)
        np.testing.assert_almost_equal(float(self._actual_vwap_df["bid_price"].first("T")), bid_price1, decimal=decimal)
        bid_price2 = (380.2 * 5.9 + 382.45 * 67.7 + 389.5 * 89.8) / (5.9 + 67.7 + 89.8)
        np.testing.assert_almost_equal(float(self._actual_vwap_df["bid_price"].last("T")), bid_price2, decimal=decimal)
        # Check `ask_price`.
        ask_price1 = (384.7 * 160.2 + 387.8 * 80.21 + 387.5 * 18.5) / (160.2 + 80.21 + 18.5)
        np.testing.assert_almost_equal(float(self._actual_vwap_df["ask_price"].first("T")), ask_price1, decimal=decimal)
        ask_price2 = (382.2 * 14.9 + 384.9 * 32.7 + 386.5 * 97.8) / (14.9 + 32.7 + 97.8)
        np.testing.assert_almost_equal(float(self._actual_vwap_df["ask_price"].last("T")), ask_price2, decimal=decimal)
    ...
    def test_resample_bid_ask_data3(self) -> None:
        ...
        np.testing.assert_almost_equal(float(self._actual_twap_df["bid_price"].first("T")), bid_price1,
                                       decimal=decimal)
        bid_price2 = (5.9 + 67.7 + 89.8) / 3
        np.testing.assert_almost_equal(float(self._actual_twap_df["bid_price"].last("T")), bid_price2,
                                       decimal=decimal)
        # Check `ask_price`.
        ask_price1 = (160.2 + 80.21 + 18.5) / 3
        np.testing.assert_almost_equal(float(self._actual_twap_df["ask_price"].first("T")), ask_price1, decimal=decimal)
        ask_price2 = (14.9 + 32.7 + 97.8) / 3
        np.testing.assert_almost_equal(float(self._actual_twap_df["ask_price"].last("T")), ask_price2, decimal=decimal)
