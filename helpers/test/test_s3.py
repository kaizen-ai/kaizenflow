...
class Test_s3_get_credentials1(hunitest.TestCase):
    def test1(self) -> None:
        res = hs3.get_aws_credentials(AM_AWS_PROFILE)
...
class Test_s3_functions1(hunitest.TestCase):
    def test_extract_bucket_from_path1(self) -> None:
        path = os.path.join(
            hs3.get_s3_bucket_path(AM_AWS_PROFILE), "tmp/TestCachingOnS3.test_with_caching1/joblib"
        )
...
class Test_s3_1(hunitest.TestCase):
    def test_ls1(self) -> None:
        file_path = os.path.join(hs3.get_s3_bucket_path(AM_AWS_PROFILE), "README.md")
    ...
    def test_glob1(self) -> None:
        ...
        s3fs = hs3.get_s3fs("am")
        file_path = os.path.join(hs3.get_s3_bucket_path(AM_AWS_PROFILE), "data/ib/metadata")
    ...
    def test_exists2(self) -> None:
        s3fs = hs3.get_s3fs("am")
        file_path = os.path.join(hs3.get_s3_bucket_path(AM_AWS_PROFILE), "README_does_not_exist.md")
        _LOG.debug("file_path=%s", file_path)
        act = s3fs.exists(file_path)
        exp = False
        self.assertEqual(act, exp)
    ...
    def test_exists3(self) -> None:
        ...
        file_path = os.path.join(
            hs3.get_s3_bucket_path(AM_AWS_PROFILE), "data/ib/metadata/symbols-2021-04-01-143112738505.csv"
        )
        _LOG.debug("file_path=%s", file_path)
        act = s3fs.exists(file_path)
        exp = True
