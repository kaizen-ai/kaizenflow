...
class Test_trim_df1(hunitest.TestCase):
    ...
    def test_types1(self):
        ...
        df = self.get_df()
        #
        act = hpandas.df_to_str(df, print_dtypes=True, print_shape_info=True, tag="df")
        exp = r"""# df=
        df.index in [4, 44]
    ...
    def test_trim_df1(self):
        ...
        # Check.
        act = hpandas.df_to_str(df_trim, print_dtypes=True, print_shape_info=True, tag="df_trim")
        exp = r"""# df_trim=
        df.index in [4, 38]
        df.columns=start_time,egid,close
        df.shape=(8, 3)
