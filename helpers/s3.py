...
def get_aws_credentials(
    ...
    force_use_aws_profile: bool = False,
    ...
    if force_use_aws_profile:
        ...
        env_var_override = any(
            ...
            for env_var in key_to_env_var.keys()
        )
    if env_var_override:
...
def _get_boto3_resource(*args: Any, **kwargs: Any) -> boto3.resource:
...
def _get_boto3_client(*args: Any, **kwargs: Any) -> boto3.client:
    aws_credentials = get_aws_credentials(*args, **kwargs)
    # _LOG.debug("%s", pprint.pformat(aws_credentials))
    s3 = boto3.client(
        "s3",
