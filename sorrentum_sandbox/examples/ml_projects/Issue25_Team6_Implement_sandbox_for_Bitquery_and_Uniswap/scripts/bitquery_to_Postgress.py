...
def postgres_push(df:pd.DataFrame):
  ...
  tran_metadata = df[["transaction_hash","trades","transaction_gas","timeInterval_minute"]]
...
def update_pull():
  ...
  cur.execute("SELECT * FROM tran_metadata ORDER BY timeinterval_minute DESC LIMIT 1")
