...
if __name__ == "__main__":
    ...
    start_timestamp_as_str = "20240321_000000"
    end_timestamp_as_str = "20240328_235900"
    db_stage = "preprod"
    ...
    # overrides.
    set_config_values = '("dag_property_config","debug_mode_config","save_node_df_out_stats"),(bool(False));("dag_property_config","debug_mode_config","save_node_df_out_stats"),(bool(False));("process_forecasts_node_dict","process_forecasts_dict","optimizer_config","backend"),(str("batch_optimizer"));("process_forecasts_node_dict","process_forecasts_dict","optimizer_config","params"),({"dollar_neutrality_penalty": float(0.0), "constant_correlation": float(0.5), "constant_correlation_penalty": float(500.0), "relative_holding_penalty": float(0.0), "relative_holding_max_frac_of_gmv": float(0.1), "target_gmv": float(1000.0), "target_gmv_upper_bound_penalty": float(0.0), "target_gmv_hard_upper_bound_multiple": float(1.05), "transaction_cost_penalty": float(0.5), "solver": str("ECOS")});("dag_config", "resample", "transformer_kwargs", "rule"),(str("5T"));("trading_period"),(str("5T"));("market_data_config","universe_version"),(str("v8.1"));("dag_property_config","force_free_nodes"),(bool(False));("dag_property_config","debug_mode_config","profile_execution"),(bool(True));("process_forecasts_node_dict","process_forecasts_dict","order_config","order_duration_in_mins"),(int(5));("process_forecasts_node_dict","process_forecasts_dict","order_config","order_type"),(str("price@start"));("portfolio_config", "pricing_method"),(str("last"))'
    # Define params.
