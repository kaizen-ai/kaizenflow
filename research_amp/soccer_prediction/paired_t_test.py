...
np.random.seed(42)  
poisson_df['predicted_random'] = np.random.choice(['home', 'away', 'draw'], size=len(poisson_df))
...
poisson_df['accuracy_poisson'] = (poisson_df['predicted_poisson'] == poisson_df['actual_outcome']).astype(int)
poisson_df['accuracy_naive'] = (poisson_df['predicted_naive'] == poisson_df['actual_outcome']).astype(int)
# Perform paired t-test.
t_stat_poisson_naive, p_value_poisson_naive = scps.ttest_rel(poisson_df['accuracy_poisson'], 
                                                             poisson_df['accuracy_naive'])
# Print results.
print(f"Poisson Model Accuracy: {poisson_df['accuracy_poisson'].mean()}")
print(f"Naive Classifier Accuracy: {poisson_df['accuracy_naive'].mean()}")
print(f"Paired t-test (Poisson vs. Naive): t-statistic = {t_stat_poisson_naive}\
