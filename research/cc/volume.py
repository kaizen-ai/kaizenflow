...
def get_rolling_volume_per_exchange(
    ...
    volume_per_exchange_per_day = volume_per_exchange_per_day.merge(rolling_volume.to_frame(),left_index=True,right_index=True)
    volume_per_exchange_per_day.rename(columns={'volume_x':'volume','volume_y':"rolling_volume"}, inplace=True)
    if display_plot:
        sns.lineplot(data=volume_per_exchange_per_day, x='date', y='rolling_volume', hue=exchange_id)
...
def get_rolling_volume_per_coin(
    ...
    rolling_volume = resampler[volume].transform(lambda x: x.rolling(window).mean())
    volume_per_coin_per_day = volume_per_coin_per_day.merge(rolling_volume.to_frame(),left_index=True,right_index=True)
    volume_per_coin_per_day.rename(columns={'volume_x':'volume','volume_y':"rolling_volume"}, inplace=True)
    if display_plot:
        sns.lineplot(data=volume_per_coin_per_day, x='date', y='rolling_volume', hue=currency_pair)
...
def compare_weekday_volumes(
    ...
    total_volume_by_weekdays = data.groupby("weekday")[volume].sum().sort_values(ascending=False)
    ...
    if plot_distr_by_weekdays:
        weekends = data[(data["weekday"] == "Saturday") | (data["weekday"] == "Sunday")]
        weekdays = data[(data["weekday"] != "Saturday") & (data["weekday"] != "Sunday")]
