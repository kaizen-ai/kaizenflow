...
def get_df_with_price_volatility(df, freq):
    resample = df.groupby(["currency_pair", pd.Grouper(key="date", freq=freq)])[
        "close"
    ].last()
    resample = resample.to_frame()
    resample["rets"] = resample.groupby(["currency_pair"]).close.pct_change()
    resample["std_18_ema_simple"] = resample.groupby(["currency_pair"])[
        "rets"
    ].transform(lambda x: x.ewm(span=18, adjust=False).std())
    rets_for_plot = resample.reset_index()
    sns.set(rc={"figure.figsize": (15, 8)})
    sns.lineplot(
        data=rets_for_plot, x="date", y="std_18_ema_simple", hue="currency_pair"
    )
    return resample
...
# %%
cplot.plot_barplot(
    ema_df_daily.groupby(["currency_pair"])["rets"]
    .std()
...
# %%
cplot.plot_barplot(
    ema_df_5min.groupby(["currency_pair"])["rets"]
    .std()
    .sort_values(ascending=False),
