...
Each dir that can be "deployed" and run should have a `devops` dir to
...
<img src="Tools_Docker_Test_figs/media/image1.png" style="width:6.26772in;height:3.09722in" />
...
<img src="Tools_Docker_Test_figs/media/image2.png" style="width:6.26772in;height:2.73611in" />
...
1.  In general we use the latest version of a package (`*`) until the
    tests fail or the system stops working

    1.  If the system fails, we freeze the version of the problematic
        packages to a known-good version to get the tests back to green
        until the problem is solved. We switch back to the latest
        version once the problem is fixed
    ...
    2.  If you need to put a constraint on the package version, follow
        ...
        and explain in a comment why this is needed making reference to
        GitHub issues
...
3.  Release the new image. To do so follow the [<span
    ...
    image</span>](#how-to-test-a-package-in-a-docker-container) section,
    use `--update-poetry` flag to resolve the dependencies
...
-   Local git tag `${repo_name}-${version}`, e.g. `cmamp-1.0.0`, is
...
-   The main difference between `dev` image and `prod` image is that
    ...
    -   source code is accessed through a bind mount for `dev` image
        (so that it can be easily modified) and copied inside the image
        for a `prod` image (since we want to package the code)
...
-   [<span class="underline">Can I run Docker-in-Docker without using
...
-   Update the changelog, i.e. `//dev_tools/changelog.txt`
    ...
    -   The changelog should be updated only after the image is
        ...
        release’s version is not higher than the latest version recorded
        in the changelog.
...
-   We are going towards the concept of "releasable dirs" (see im,
    optimizer). If there is a dir with devops, then that dir runs inside
    a container
...
-   Rollback an image
    ...
    -   We rarely move the dev / prod tag back, but rather users needs
        to docker pull an older image and pass --base_name --stage and
        --version to docker_{bash, cmd, jupyter}
