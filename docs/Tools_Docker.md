...
- Approach 2: If we think we need to add new packages only for running infra scripts then
...
- A docker image is built from a `Dockerfile`. The image is then used to run a
  Docker container.
![](docs/Tools_Docker_figs/image2.png){width="6.267716535433071in"
...
![](docs/Tools_Docker_figs/image1.png){width="6.267716535433071in"
...
- When the Docker container starts the current dir is optimizer helpers, core is mounted in the same dir
