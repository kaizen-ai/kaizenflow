...
- A way to check that the branch is sane is the following:
  ...
  `bash > git fetch > git checkout &lt;BRANCH> > git log origin/master..HEAD` shows only commits made by you or, if you are not the only one working on the branch, only commits belonging to the branch with the same `PTaskXYZ`
    - E.g., if George is working on `PTask275` and sees that something funny is going on:
      ```
...
- To see the actual changes in a branch you can't do (Bad) \
  `> git diff master..HEAD` since `git diff` compares two commits and not a range of commits like `git log` (yes, Git has a horrible API)
