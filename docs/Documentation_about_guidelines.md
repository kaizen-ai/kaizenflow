...
- Run linter in order to build TOC automatically
    - Detailed linter instructions at [First Review Process](First_review_process.md)
    - Currently linter does puts TOH above all the markdown text, so put a file name on top after the linter is run
