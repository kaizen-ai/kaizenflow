...
-   A notebook can be used for:
    ...
    -   analysis
        -   the notebook should always work so we need to treat it as part of
            the code base
        -   we might want to add unit tests for it
    -   tutorial / gallery
        -   shows how some code works (e.g., functions in
...
-   Each notebook needs to follow a clear and logical flow, e.g.,
    ...
    -   load data
        -   for each data structure it's good to show a snippet, e.g.,
            df.head(3)
...
-   Use the extension for navigating the notebook (see our suggestions for
    Jupyter plug-ins)

-   Keep related code and analysis close together so:
    -   readers can understand the logical flow
    -   one could "easily" split the notebook in parts (e.g., when it becomes
        too big)
...
-   E.g., put all the imports in one cell at the beginning, so with one cell
    ...
    -   Compare this with the case where the imports are randomly sprinkled in
        the notebook, then you need to go execute them one by one if you
        re-initialize the notebook

-   For the same reason group functions in one cell that you can easily
    re-execute
...
-   Another approach is to factor out the code in functions and simplify the
    flow

## Do not cut & paste code

-   It's _never_ a good idea
-   It takes more time to clean up cut & paste code than doing right in the
    first place
...
-   If it takes too long to compute the statistics on the entire data set,
    report the problem and we can think of solutions
...
-   If you are using data from a file (e.g.,
    `/data/wd/RP_1yr_13_companies.pkl`), explain in a comment how the file was
    generated
...
-   Warnings can't be ignored since they indicate that:
    ...
    -   if we don't fix the issue now, the next time we create a conda
        environment the code might either break or (even worse) have a different
        behavior, i.e., silent failure
    ...
    -   This is a typical pandas warning telling us that we created a view on a
        ...
        through the view
    -   This is dangerous since it can create unexpected side effects and
        coupling between pieces of code that can be painful to debug and fix

-   If you have warnings in your code or notebook you can't be sure that the
    code is doing exactly what you think it is

    -   For what we know your code might be deleting your hard-disk, moving
        money from your account to mine, ..
    -   You don't ever want to program by coincidence

-   Typically the warnings are informative and tell us what's the issue and how
    to fix it, so please fix your code
    -   If it's not obvious how to interpret or fix a warning file a bug,
        reporting clearly a repro case and the error message

## Make cells idempotent

-   Try to make a notebook cell being executed multiple times without changing
    its output value, e.g.,
...
-   **_Bad_**
    ...
    -   This computation is not idempotent, since if you execute it multiple
        times is going to increment the column "id" at every iteration
...
-   **_Good_**
    ...
    -   In this way it's easy to add another stage in the pipeline without
        changing everything
    -   Of course the names `tmp_1`, `tmp_2`, ... are a horrible idea since they
        are not self-explanatory and adding a new stage screws up the numbering

## Always look at the discarded data

-   Filtering the data is a risky operation since once the data is dropped,
    nobody is going to go back and double check what exactly happened
-   Everything downstream (all the results, all the conclusions, all the
...
-   Make absolutely sure you are not dropping important data
    -   E.g., is the distribution of the data changed in the way you would
        expect?
...
-   Cleaning up / commenting / untangling the code can help reason carefully
    about the assumptions to find issues
-   The notebook becomes more streamlined and easy to understand since now it's
    a sequence of functions `do_this_and_that` and presenting the results
...
-   If that's not possible or convenient make individual plots with different
    scales and add a plot with multiple graphs inside on the same axis (e.g.,
    with y-log)
