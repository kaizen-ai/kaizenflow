...
- The goal is to obtain WIND Commodity DB metadata
- WIND has a restriction on the number of metadata files a user can load, so we:
    - automated the process of "walking" WIND menu and taking screenshots of the
      metadata, and
    - used OCR to extract text from the screenshots
...
- TODO(gp): Are the data on S3 now? Can we update the link?

# 2. OCR for Automax results

- We tried Tesseract and Amazon's Textract for OCR
  - Textract showed much better results, so we used it for recognizing WIND data
    here and below.
...
- The recognized data is located at
  ...
  ```
  > head -5 /s3/default00-bucket/wind/all_commodities_mapped_v3.csv
  ,,values
  Precious Metal_Gold,Upstream,"['Gold Reserves', 'Macro Environment']"
  Precious Metal_Gold,Midstream,"['Gold Reserves', 'Mineral and Regenerated', 'Gold Production Cost', 'Imports and Exports', 'Futures Market', 'Spot Market', 'ETF Position', 'Listed Company']"
  Precious Metal_Gold,Downstream,"['Subitem Demand', 'Region Demand']"
  Precious Metal_Silver,Upstream,"['Silver Reserves', 'Macro Environment']"
  ```
...
- Using the mapping for the WIND menu obtained above, we take screenshots of four
  levels of Commodity DB menu (up to `Commodity Index`and `Commodity Market`)
...
- The WIND terminal stops responding after running for several hours, so we had
  to run the script multiple times
  - The history of the runs is here
...
- After step 3. we have images of the metadata and we had to extract the data
  using OCR
...
- The resulting CSV that combines data from screenshots in both aforementioned
  ...
  ```
  > head/s3/default00-bucket/wind/datasets/Wind_terminal/WIND_metadata_table.csv
  ...
  USA: Change in Employrnent Level (CEL): Nonfarm: Total: SA,Monthly,1000 persons,Feb-1939,Nov-2019,2019/12/06,Bureau of Labor Statistics,USA,G0000070,(0_0_1_1)
  USA: Employees on Nonfarrn Payrolls (ENP): Total: SA,Monthly,1000 persons,Jan-1939,Nov-2019,2019/12/06,Bureau of Labor Statistics,USA,G0002446,(0_0_1_1)
  [ZIUSA: Existing Home Sales: Annual Rates: SA.,Monthly,10000 units,Jan-1999,Nov-2019,2019/12/20,National Association of Realtors,USA,G0003745,(0_0_1_1)
  [ZIUSA: New Houses Sold: Annual Rates: SA.,Monthly,1000 units,Jan-1963,Oct-2019,2019/11/27,The Department of Commerce,USA,G0003650,(0_0_1_1)
  [ZIUSA: New Privately-owned Housing Units Started: Annual Rates: SA,Monthly,1000 units,Jan-1959,Nov-2019,2019/12/18,The Department of Commerce,USA,G0003622,(0
  _0_1_1)
  ```
...
## `Automax` result
- It is a dataframe of the following format:
...
## `Automax_indicators` result
- It is a dataframe with the following format:
...
- In `screenshot_number`, e.g., `(0_0_1_1)`:
...
- The output table is located at
  ...
  ```
  > head /s3/default00-bucket/wind/datasets/Wind_terminal/WIND_metadata_table_with_index.csv
  0,1,2,3,Name,Frequency,Unit,Start Date,End Date,Update,Source,Country,ID,screenshot_number
  Precious Metal_Gold,Upstream,Macro Environment,1,USA: Continuing Claims: SA,Weekly,person,Ol-Jan 1967,07-Dec 2019,2019/12/20,Bureau of Labor Statistics,USA,G0002434,(0_0_1_1)
  Precious Metal_Gold,Upstream,Macro Environment,1,USA: Change in Employrnent Level (CEL): Nonfarm: Total: SA,Monthly,1000 persons,Feb-1939,Nov-2019,2019/12/06,Bureau of Labor Statistics,USA,G0000070,(0_0_1_1)
  Precious Metal_Gold,Upstream,Macro Environment,1,USA: Employees on Nonfarrn Payrolls (ENP): Total: SA,Monthly,1000 persons,Jan-1939,Nov-2019,2019/12/06,Bureau of Labor Statistics,USA,G0002446,(0_0_1_1)
  ```

- We had to merge multiple overlapping CSV files, removing the redundancy

# 7. Cleaning up OCR results
