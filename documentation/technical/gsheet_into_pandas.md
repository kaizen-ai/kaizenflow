...
- To use the library on the server, the downloaded JSON with the credentials
  ...
  ```bash
  > mv ~/Downloads/client_secret_4642711342-ib06g3lbv6pa4n622qusqrjk8j58o8k6.apps.googleusercontent.com.json ~/.config/gspread_pandas/google_secret.json
  > chmod 600 ~/.config/gspread_pandas/google_secret.json
  ```
  and on the server, e.g.,
  ```bash
  `/wd/{your_username}/.config/gspread_pandas/google_secret.json`.
  ```
