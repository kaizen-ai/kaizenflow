...
- The close price from one bar is often different from the open price of the next
  bar
...
- Kibot adjusted data matches Yahoo historical prices except for the daily
  closing price which is the last transaction during the regular trading session,
  while for Yahoo might be a transaction after the close
...
- The value price * volume is independent of adjustment since it represents the
