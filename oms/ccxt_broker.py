...
class CcxtBroker(ombroker.Broker):
    def __init__(
        self, exchange, universe_version: str, mode: str, portfolio_id: str, *args: Any, **kwargs: Any
    ) -> None:
    ...
    def _assert_order_methods_presence(self) -> None:
        ...
        for method in methods:
            if not self._exchange.has[method]:
                _LOG.error(f"Method {method} is unsupported for {self._exchange_id}.")
                abort = True
        if abort:
            raise ValueError(f"The {self._exchange_id} exchange is not fully supported for placing orders.")
    ...
    def _build_asset_id_to_symbol_mapping(self, universe_version: str) -> Dict[int, str]:
        """
        Build asset id to full symbol mapping.
        """
        # Get full symbol universe.
        full_symbol_universe = imvcounun.get_vendor_universe("CCXT", version=universe_version, as_full_symbol=True)
        # Filter symbols of the exchange corresponding to this instance.
        full_symbol_universe = list(filter(lambda s: s.startswith(self._exchange_id), full_symbol_universe))
        # Build mapping.
        asset_id_to_full_symbol_mapping = imvcuunut.build_numerical_to_string_id_mapping(full_symbol_universe)
        # Change mapped values to be symbol only (more convevient when placing orders)
        asset_id_to_symbol_mapping = {id_: imvcufusy.parse_full_symbol(fs)[1].replace("_", "/") for id_, fs in asset_id_to_full_symbol_mapping.items()}
        return asset_id_to_symbol_mapping # type: ignore[no-any-return]
