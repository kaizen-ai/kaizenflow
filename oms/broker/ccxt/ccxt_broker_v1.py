...
class CcxtBroker_v1(obcaccbr.AbstractCcxtBroker):
    """
    ...
    def submit_twap_orders(
        ...
        *,
        ...
        while True:
            ...
            order_dfs = hasynci.run(self._submit_twap_child_orders(
                parent_orders_tmp,
                parent_order_ids_to_child_order_shares,
                order_dfs,
                execution_freq,
        ...
        hasynci.run(hasynci.async_wait_until(
            self._get_wall_clock_time().ceil("T"), self._get_wall_clock_time
        ), close_event_loop=False)
    ...
    def _log_child_order_fills(
        ...
        # Generate file names.
        start_timestamp_str = CcxtBroker_v1.timestamp_to_str(start_timestamp)
        end_timestamp_str = CcxtBroker_v1.timestamp_to_str(end_timestamp)
        ...
        *,
        ...
        for symbol in symbols:
            if end_timestamp - start_timestamp <= day_in_millisecs:
                ...
                symbol_trades = self._async_exchange.fetchMyTrades(
                    symbol=symbol,
                ...
                for timestamp in range(
                    ...
                    day_trades = self._async_exchange.fetchMyTrades(
                        symbol=symbol,
                        since=timestamp,
    ...
    async def _submit_twap_child_orders(
        ...
        for parent_order in parent_orders_tmp:
            ...
            if self._skip_child_order_if_needed(
                parent_order, parent_order_ids_to_child_order_shares
            ):
                continue
            ...
            if is_submitted_order:
                ...
                child_order_ccxt_id = self._get_ccxt_id_from_child_order(
                    child_order
                )
                hdbg.dassert_not_in(
