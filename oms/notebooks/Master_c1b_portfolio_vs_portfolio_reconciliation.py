...
# %%
list_conf1 = cc.split("\n")
config_list2 = build_reconciliation_configs()
config2 = config_list2[0]
cc2 = str(config2)
list_conf2 = cc2.split("\n")

# %%
import difflib
b = difflib.Differ()
a = b.compare(list_conf1, list_conf2)
print('\n'.join(a))

# %% language="javascript"
# IPython.OutputArea.auto_scroll_threshold = 250;
...
diff = difflib.HtmlDiff().make_file(list_conf1, list_conf2)
display(HTML("<style>.rendered_html tr, .rendered_html th, .rendered_html td{ text-align:left; max-width: 700px; !important; }</style>"))
display(HTML(diff))
