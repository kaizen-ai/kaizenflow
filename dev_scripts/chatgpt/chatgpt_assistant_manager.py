...
instructions = hgptinstr.instructions
...
def _parse() -> argparse.ArgumentParser:
    ...
    group.add_argument('-c', '--create', dest='create_name',
                    help='name of the assistant to be created')
    group.add_argument('-e', '--edit', dest='edit_name',
                    help='name of the assistant to be edited')
    group.add_argument('-d', '--delete', dest='delete_name',
    ...
    parser.add_argument('-m', '--model', dest='model',
                    help='model used by the assistant')
    parser.add_argument('-i', '--instruction_name', dest='instruction_name',
    ...
    parser.add_argument('--r', '--retrieval_tool', dest='retrieval_tool', action=argparse.BooleanOptionalAction,
                    help='enable the retrieval tool. Use --no-r to disable')
    parser.add_argument('--c', '--code_tool', dest='code_tool', action=argparse.BooleanOptionalAction,
                    help='enable the code_interpreter tool. Use --no-c to disable')
    parser.add_argument('--function', dest='function', action='store',
...
def _main(parser: argparse.ArgumentParser) -> None:
    ...
    if args.create_name:
        ...
        hgpt.add_files_to_assistant_by_name(args.create_name, args.input_file_paths)
    elif args.edit_name:
        assistant_id = hgpt.get_assistant_id_by_name(args.edit_name)
        tools = []
        if args.retrieval_tool:
            tools.append({'type': "retrieval"})
        if args.code_tool:
            tools.append({'type': "code_interpreter"})
        hgpt.update_assistant_by_id(
            assistant_id=assistant_id,
            instructions=instructions[args.instruction_name],
            name=args.new_name,
            tools=tools,
            model=args.model,
            file_ids = [hgpt.get_gpt_id(path) for path in args.input_file_paths]
        )
    elif args.delete_name:
        assistant_id = hgpt.get_assistant_id_by_name(args.delete_name)
        hgpt.delete_assistant_by_id(assistant_id)
