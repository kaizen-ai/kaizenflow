...
produce nice stat file in `.json` format.
"""
...
class PoetryDebugger:
    def __init__(self, debug_mode: str, max_runtime_minutes: int) -> None:
        ...
        """
        self.debug_mode: str = debug_mode
...
class PoetryDebuggerAnalyzer:
    ...
    def check_log_validity(self, log_file: str) -> str:
        """
        ...
        if locked:
            # Get total seconds.
            time_info = re.search(
                r"Complete version solving took (.*?) seconds", log_file
            )
            if not time_info:
                # If run is too fast, there will be information only for one iteration.
                time_info = re.search(
                    r"Version solving took (.*?) seconds", log_file
                )
                if not time_info:
    ...
    def run_log_analyzer(self) -> None:
        ...
        """
        working_directory = PoetryDebugger.get_debug_poetry_dir()
        # Collect all logs.
        ...
        for log_path in log_paths:
            ...
            time_info = self.check_log_validity(log_file)
            # Update stats.
            self.update_poetry_run_stats(debug_mode_dirs, time_info)
        # Save stats.
...
def _parse() -> argparse.ArgumentParser:
    ...
    # TODO(Nikola): Args for `PoetryDebuggerAnalyzer.stats` additional metadata?
...
def _main(parser: argparse.ArgumentParser) -> None:
    ...
    try:
        ...
        poetry_debugger_analyzer.run_log_analyzer()
