...
import dataflow.core.utils as dtfcorutils
...
class RealTimeDataSource(dtfcore.DataSource):
    ...
    def _get_data(self) -> None:
        ...
        if self._multiindex_output:
            self.df = dtfcorutils.convert_to_multiindex(self.df, self._asset_id_col)
...
class HistoricalDataSource(dtfcore.DataSource):
    ...
    def _get_data(self, intervals: dtfcore.Intervals) -> pd.DataFrame:
        ...
        if self._multiindex_output:
            df = dtfcorutils.convert_to_multiindex(df, self._asset_id_col)
        return df
