...
if [[ 1 == 0 ]]; then
 /app/dataflow/model/run_experiment.py \
     ...
     --config_builder 'dataflow.pipelines.examples.example1_configs.build_tile_configs("kibot_v1-top1.5T.JanFeb2020")' \
 ...
 /app/dataflow/model/run_experiment_stub.py --experiment_builder 'dataflow.model.master_experiment.run_tiled_experiment' --config_builder 'dataflow.pipelines.examples.example1_configs.build_tile_configs("kibot_v1-top1.5T.JanFeb2020")' --config_idx 0 --dst_dir /app/dataflow/pipelines/examples/test/outcomes/Test_Example1_TiledBacktest.test1/tmp.scratch/run_model -v INFO 2>&1 | tee log.txt
