...
import datetime
import vendors_amp.kibot.data.load.sql_data_loader as loa
...
class TestSqlWriterBackend1(hut.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        writer = vksqlw.SQLWriterBackend(dbname, user, password,
                                         host, port)
    ...
    def setUp(self) -> None:
        ...
        # Create loader.
        self._loader = loa.SQLKibotDataLoader(dbname, user, password, host, port)
    ...
    def test_get_trade_symbol_id1(self) -> None:
        """
        Test correct mapping from symbol/exchange pair to internal trade symbol id.
        """
    ...
    def _prepare_tables(
        cls,
        writer: vksqlw.SQLWriterBackend
    ) -> None:
        """
        ...
        with writer.conn as conn:
            with conn.cursor() as curs:
                ...
                for trade_symbol_id in [10, 11, 12, 13]:
                    ...
                    writer.insert_bulk_daily_data(generated_data.drop(columns=["datetime"]))
                    writer.insert_bulk_minute_data(generated_data.drop(columns=["date"]))
    ...
    def _generate_test_data(seed: int) -> pd.DataFrame:
        ...
        df = pd.DataFrame({
            ...
            "date": [datetime.date(2021, 1, i+1).isoformat() for i in range(nrows)],
            "datetime": [datetime.datetime(2021, 1, 1, 1, i+1, 0).isoformat() for i in range(nrows)],
