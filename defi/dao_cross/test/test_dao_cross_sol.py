...
def test_on_swap_time(daocross, base_token, price_oracle):
    # Add some buy orders.
    ...
    #base_token.transfer(accounts[3], 10**18, {"from": accounts[0]})
    #base_token.transfer(accounts[4], 10**18, {"from": accounts[0]})
    base_token.approve(daocross, 10**18, {"from": accounts[3]})
    base_token.approve(daocross, 10**18, {"from": accounts[4]})
    daocross.sellOrder(base_token.address, 10**18, 1900, accounts[3], {"from": accounts[3]})
    daocross.sellOrder(base_token.address, 10**18, 2000, accounts[4], {"from": accounts[4]})
