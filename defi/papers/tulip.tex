...
% TODO(gp): @paul At this point I would just use the most general "Tulip: A
% decentralized protocol for token exchange" and say "Tulip" is just the name,
% because it's cute.
...
\subsection{Limit Order Book (LOB)}
	...
	\item Liquidity pooling across tokens: Low
	      \begin{itemize}
		      \item LOBs only allow exchanging one type of token with another,
                artificially separating liquidity
	      \end{itemize}
	\item Liquidity pooling across time: High
	      \begin{itemize}
	...
	\item Trading efficiency: Medium
	      \begin{itemize}
              \item Although the continuous matching 
                The trading efficiency in matching trades is the same as the
                lit exchange on which price discovery is peformed (e.g., a
                centralized exchange)
	      \end{itemize}
	\item Transparency: Yes
