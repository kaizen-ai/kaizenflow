...
% ===============================================================================
\subsection{Building a demand curve with limit orders}
...
To convert $Q_+(p)$ to a set of limit orders, we begin with the highest price
...
though in practice minimal tick increments. The key assumption, almost always
satisfied in practice, is that cumulative supply and demand are monotonic in
price.
...
We propose maximizing the total quantity of token swapped, a simple linear
constraint.
...
the optimal values of the objective are generally different).
...
the rescaled token, which gets rescaled linearly. The property of being
a local maximum is preserved by this transformation, and since any local
...
\begin{problem}[DaoCross token exchange]
...
\begin{problem}[DaoSwap token exchange with additional constraints]
Let..
\end{problem}
...
% TODO(Paul): This section discusses a special case of DaoCross.
