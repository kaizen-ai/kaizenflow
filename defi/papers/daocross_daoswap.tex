...
orders. The ability to express intent to buy or sell an asset in exchange for
another using limit orders is foundational in markets, as supply and demand are
naturally expressed in terms of quantity, and matching supply and demand
determines price. Limit orders provide flexibility to traders, while
simultaneously clearly delineating maximum possible exposure.

The popularity of centralized exchanges in cryptocurrency trading may be
attributable in part to the familiar interface (limit orders) and familiar
market microstructure (limit order books, or LOBs). However, the ethos of
decentralized finance, the opportunity to reimagine how financial components
or functions may operate, and certain high-profile centralized exchange
issues, have all led to and spurred on a flurry of innovation in the realm of
decentralized token exchange.

Some have pursued creating decentralized limit order books. A significant
drawback of this direction is on-chain computational expense.
Others have created markets that rely upon automated market makers (AMMs).
While AMMs provide certain advantages, such as in containing computational
expenses, they suffer from drawbacks such as impermanent loss and interfaces
that differ from the limit order interface.

The approach we introduce preserves the useful interface of traditional
markets (limit orders), embraces the advantages of decentralization and
transparency, and circumvents the major disadvantages of alternative
...
% TODO(gp): Maybe we should just say "in traditional finance people think in terms
% of limit orders", "people want to swap tokens independently of the direction".
% The general solution is DaoSwap.
% Or maybe explain how traditional markets work since they have been evolving for
% long time with certain use cases.
...
% TODO(gp): Can the example be called with a reference to the section, instead
% of having a different numbering scheme?
...
\]
means
% TODO(gp): I understand the intent of pairing variables in the tuples and
% natural language. I would use buy and not \buy here since we are using natural language
% In fact we don't use \texttt{1.0} in the text. I'm ok with leaving BTC and ETH
% in teletype since those are symbols like $ and it's a coincidence that teletype
% is used for both code and our symbols.
...
\subsubsection{Extracting attributes from orders}
Given an order $o_i =$ \texttt{(\buy, q, A, lb, B)} we indicate with:
\begin{itemize}
	\item $action(o_i)$ the desired action (e.g., $\buy$)
	\item $q_{base}(o_i)$ the maximum desired quantity $q$
	\item $lb(o_i)$ the limit price $lb$
	\item $base(o_i)$ the base token $A$
	\item $quote(o_i)$ the quote token $A$
\end{itemize}

\subsubsection{Inequalities for buy order}
An order of the form $o_i =$ \texttt{(\buy, q, A, lb, B)} means ``buy $q$ units of
token A in exchange for token B with a limit price up to $lb$ B per unit of A"
and it corresponds to the following constraint on realized quantity exchanged
and clearing exchange rate:
\begin{equation*}
	(p_{B\_per\_A})^* \leq lb(o_i) \land
...
An order of the form $o_i =$ \texttt{(\sell, q, A, lb, B)} means ``sell $q$ units of
token A paying in token B with a limit price of at least to $lb$ B per unit of A"
and it corresponds to the following constraint on realized quantity exchanged
and clearing exchange rate:
\begin{equation*}
	(p_{B\_per\_A})^* \geq lb(o_i) \land
	(0 \le q_{A}^*(o_i) \geq q(o_i)) \lor
	(q_A^*(o_i) = 0)
\end{equation*}
...
\begin{exmp}[Inequalities for buy market order]
...
buy / sell orders for A and B tokens into buy / sell orders for one of the
token only, according to the transformation below.
