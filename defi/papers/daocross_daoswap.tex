...
orders. A limit order expresses a commitment to buy or to sell an asset, up to
...
flexible in that it allows participants to construct personalized supply and
demand curves using multiple limit orders. The elegance follows from the
simplicity and ease-of-use of the interface.

In any case, limit orders and their aggregation into limit order books (LOBs)
are wildly popular. They are the backbone of traditional financial markets
and dominate cryptocurrency trading on centralized exchanges. Interestingly,
however, limit orders do not currently play a prominent role in the realm of
...
A key obstacle to using limit order books in decentralized exchange is cost of
computation. Continual submission and cancelation of orders incurs costs, as
does the ongoing process of matching supply and demand. X addresses these
core issues primarily in two ways: (1) discretizing time; and (2) scaling
with layer 2 solutions. By addressing these issues, we combine the novel
