...
orders. It brings together the advantages of trading efficiency and ease-of-usj
...
the swap. At the same time orders are matched in many-to-many relationships.
...
\[
	c_i := \left(
	p^*_{\tau_i\_per\_\pi_i} \leq \mathcal{I}(a_i) \cdot p_i
	\right)
\]
where $p^*_{\tau_i\_per\_\pi_i}$ is the unique clearing price for
...
\[
    E = \bigcup_{\substack{i \in \{1, \ldots, n\} \\ \pi_i, \tau_i \in T}}(\pi_i, \tau_i)
\]
Let $C$ denote the collection of all elementary circuits of $\Gamma$.
For an elementary circuit $\gamma \in C$, consecutively enumerate the vertices
(with starting place arbitrary) by $c_j$ for $j = 1, \ldots, |\gamma|$. The
\emph{no arbitrage} constraints states that, for each $\gamma \in C$, we have
\[
    p^*_{{c_1\_per\_c_{|\gamma|}}} \cdot
    \prod_{j = 1}^{|\gamma| - 1}p^*_{{c_{j + 1}}\_per\_c_j}
    = 1
\]
...
\[
	\tilde{q}_i^* = q_i^* \cdot p_{\tau_i\_per\_\pi_i}^*
\]
...
\[
	\sum_i \mathcal{T}_u(\pi_i) \cdot q^*_i
	=
	- \sum_i \mathcal{T}_u(\tau_i) \cdot \tilde{q}_i^*
\]
...
\begin{equation}
	\begin{cases}
		c_i \land (0 \leq q_i^* \leq q_i) \lor (q_i^* = 0)
        ...
        = 1
		 & \forall \gamma \in C \\
        \sum_{j = 1}^n \mathcal{T}_u(\pi_i) \cdot q^*_i
        =
        - \sum_{j = 1}^n \mathcal{T}_u(\tau_i) \cdot \tilde{q}_i^*
         & \forall u \in T
	\end{cases}
\end{equation}
Note that $n$ is the number of orders and $|T| \leq n$. So first two
collections of constraints and the last collection of constraints grow at most
linearly in the number of orders and so are $O(n)$.
Claim: the number of elementary cycles is also bounded by $n$ (consider proof
by induction, looking at how many cycles can be added with each additional
edge).
