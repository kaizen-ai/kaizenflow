...
We require that the following \emph{no arbitrage}
...
\begin{equation}
	...
	\end{cases}
\end{equation}
TODO(Paul): Determine the minimal number of no-arbitrage constraints that need
to be verified. The number of no-arbitrage constaints is on the order of
$O(|T|^3)$, whereas the number of other constraints grows at most $O(n)$.
...
Switch to the more natural cumulative quantity up to a given (max) price $p$
(as in \cite{Yo20}).
