...
% TODO(gp): Do we spell Deloc or DELOC?
% TODO(gp): Should we switch to DELOCK?
...
to recent scandals involving CEXs (e.g., FTX bankruptcy), regulatory
...
and discussed in detail later in Section \ref{AlSoToSw}.
...
In the sequel we propose an exchange mechanism valid for any collection of
...
$\{-1, +1\}$, with sign determined by $o$ in the following way: if
$a$ (the action) of $o$ is to buy, then set $a_\pi = -1$ (the buy
...
When an order $o$ is executed, quantity exchange (of both base and quote
...
corresponding limit order variable.
...
\begin{itemize}
	...
	\item $q_\tau^*$ be the quantity of the quote token $\tau$ exchanged in
          th execution of order $o$
	\item $p_\pi^*$ denote the actual price of the base token with respect
...
Throughout this section, let $\{o_i\}_1^n$ denote a collection of $n$ limit
...
% TODO(gp): To simplify let's use the notation with the asterisk to represent
% the actual quantity, e.g., $\pi_i^*$
...
them until market pressures eliminated the opportunity. Note, though, that
...
\[
    ...
    \end{bmatrix}^T
\]
Then the limit order quantity constraint is expressed in matrix form via
\[
    Q x \leq q
\]
...
elsewhere. Then the limit order price constraint may be expressed as
\[
    P x \leq 0
\]
