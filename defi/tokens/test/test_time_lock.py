...
def mock_token(MockERC20, accounts):
    return MockERC20.deploy("Mock Token", "MCK", 1000,10000, {'from': accounts[1], "gas_price": "60 gwei"})
...
def test_addAmount(time_lock, accounts):
    ...
    time_lock.addAmount(accounts[2], new_amount, {"from": accounts[0], "gas_price": "60 gwei"})
...
def test_releaseTokens(time_lock, accounts, mock_token):
    ...
    mock_token.transfer(time_lock.address, amount, {"from": accounts[1], "gas_price": "60 gwei"})
...
def test_withDrawTokens(time_lock, accounts, mock_token):
    amount = 3* 10**18
    mock_token.transfer(time_lock.address, amount, {"from": accounts[1], "gas_price": "60 gwei"})
    time_lock.withdrawTokens(amount, {"from": accounts[0], "gas_price": "60 gwei"})
