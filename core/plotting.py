...
_COLORMAP = Union[str, matplo.colors.Colormap]
...
def get_multiple_plots(
    ...
    *args: Any,
    **kwargs: Any,
) -> Tuple[matplo.figure.Figure, np.array]:
    """
...
def plot_value_counts(
    ...
    Same interface as cplott_count_series() but computing the count of
    the given series `srs`.
    """
    # Compute the counts.
...
def plot_cols(
    ...
    mode: Optional[str] = None,
    axes: Optional[List[matplo.axes.Axes]] = None,
...
def plot_spectrum(
    ...
    for idx, col in enumerate(signal.columns):
        ...
        axes = cast(List, axes)
        ax1 = axes[idx][0]
        f_pxx, Pxx = scipy.signal.welch(data)
        ...
        ax2 = axes[idx][1]
        f_sxx, t, Sxx = scipy.signal.spectrogram(data)
...
def plot_dendrogram(
    ...
    **kwargs: Any,
    ...
    :param figsize: if nothing specified, basic (20,5) used
    :param kwargs: kwargs for `scipy.cluster.hierarchy.dendrogram`
    """
    # Look at:
...
def plot_pnl(
    ...
    cmap = matplo.cm.get_cmap(colormap)
...
def plot_qq(
    ...
    scipy.stats.probplot(x_plot, dist=dist, plot=ax)
...
def plot_sharpe_ratio_panel(
    ...
    for freq in frequencies:
        ...
        if len(resampled_log_rets) == 1:
            _LOG.warning(
                "Resampling to freq='%s' is blocked because "
                "resampled series has only 1 observation",
                freq,
            )
            continue
...
def _maybe_add_events(
    ax: matplo.axes.Axes, events: Optional[List[Tuple[str, Optional[str]]]]
) -> None:
    """
