...
def compute_bar_metrics(
    ...
    hpandas.dassert_time_indexed_df(
        positions, allow_empty=True, strictly_increasing=True
    )
    hpandas.dassert_time_indexed_df(
        flows, allow_empty=True, strictly_increasing=True
    )
    hpandas.dassert_time_indexed_df(
        pnl, allow_empty=True, strictly_increasing=True
    )
    hpandas.dassert_axes_equal(positions, pnl)
    hpandas.dassert_axes_equal(flows, pnl)
    ...
    if compute_extended_stats:
        ...
        stats_dict["wnl"] = wnl
        # Compute transaction costs based on flows and half of the quoted
